...
print " "
print "STATEMENT OF FINANCIAL POSISION"
...
print " "
print "Extended insights:"
opening_stock = float(OS)
purchases = float(PU)
closing_stock = float(CS)
sr = float(SA)
...
with open('Results.csv', 'w') as csvfile:
    fieldnames = ['Statement of Finanacial Posision', 'Company', 'Sector', 'Current Liabilities', 'Non-Current Liabilities', 'Current Assets', 'Non-Current Assests', 'Equity']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader ()
    writer.writerow ({'Statement of Finanacial Posision': ' ', 'Company': CN1, 'Sector': S1, 'Current Liabilities': CL, 'Non-Current Liabilities': NCL, 'Current Assets': CA, 'Non-Current Assests': NCA, 'Equity': EQ})
    writer.writerow ({})
    
    fieldnames = ['Income Statement', 'Company', 'Sector', 'Purchases', 'Interest Payables', 'Sales', 'Expenses', 'Interest Receivables', 'Opening Stock', 'Closing Stock']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader ()
    writer.writerow ({'Income Statement': ' ', 'Company': CN2, 'Sector': S2, 'Purchases': PU, 'Interest Payables': IP, 'Sales': SA, 'Expenses': EX, 'Interest Receivables': IR, 'Opening Stock': OS, 'Closing Stock': CS})

    fieldnames = ['Extended Insights', 'Cost of Sales', 'Gross Profit']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader ()
    writer.writerow ({'Extended Insights': ' ', 'Cost of Sales': result_cos, 'Gross Profit': result_gp})
print "......................."
